...
const debug = debugFactory("ux-eslint-parser");
...
function replaceAliasParens(code) {
    if(code.indexOf('in') < 0) {
        code = `($idx, $item) in ${code}`
    }
...
const DIRECTIVE_NAME = ['for', 'if', 'elif', 'else', 'show', 'tid', 'onclick', '@click', 'is'];
const DT_DD = /^d[dt]$/u;
...
function handlerDirectiveValue(value) {
    return value.replace(/^\{{2}/gi, '').replace(/\}{2}$/gi, '');
}
class Parser {
    get text() {
        return this.tokenizer.text;
    ...
    processAttribute(node, namespace) {
        ...
        if ((this.expressionEnabled ||
            ...
            (DIRECTIVE_NAME.indexOf(attrName) >= 0 ||
                attrName === "slot-scope" ||
                (tagName === "template" && attrName === "scope"))) {
                    if(node.value && node.value.value) {
                        node.value.value = handlerDirectiveValue(node.value.value)
                    }
...
function isUxFile(code, options) {
    const filePath = options.filePath || "unknown.js";
    return path.extname(filePath) === ".ux" || STARTS_WITH_LT.test(code);
}
