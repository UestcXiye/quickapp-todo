...
function isUsingIterationVar (vFor, vBindKey) {
  ...
  return references.some(reference =>
    variables.some(variable => variable.id.name === reference.id.name && variable.kind === 'v-for')
  )
}
...
function checkChildKey (context, vFor, child) {
  ...
  if (childFor != null) {
    ...
    const usedInFor = childForRefs.some(cref =>
      variables.some(variable => cref.id.name === variable.id.name && variable.kind === 'for')
    )
...
function checkKey (context, vFor, element) {
  ...
  // 暂时注释掉，因为快应用会默认使用数组索引($idx)作为唯一 Id
  // if (utils.isCustomComponent(element) && vBindKey == null) {
  //   context.report({
  //     node: element.startTag,
  //     loc: element.startTag.loc,
  //     message: "Custom elements in iteration require 'tid' directives."
  //   })
  // }
  if (vBindKey != null && !isUsingIterationVar(vFor, vBindKey)) {
    context.report({
      ...
      loc: vBindKey.loc,
      message:
        "Expected 'tid' directive to use the variables which are defined by the 'for' directive."
    })
  }
}
...
module.exports = {
  meta: {
    type: 'problem',
    docs: {
      description: 'enforce valid `for` directives',
      category: 'essential',
      ...
      "VAttribute[directive=true][key.name.name='for']" (node) {
        ...
        if (node.key.argument) {
          context.report({
            node,
            loc: node.loc,
            message: "'for' directives require no argument."
          })
        }
        if (node.key.modifiers.length > 0) {
          context.report({
            node,
            loc: node.loc,
            message: "'for' directives require no modifier."
          })
        }
        if (!utils.hasAttributeValue(node)) {
          context.report({
            node,
            loc: node.loc,
            message: "'for' directives require that attribute value."
          })
          return
        }
        ...
        if (expr.type !== 'VForExpression') {
          context.report({
            node: node.value,
            loc: node.value.loc,
            message: "'for' directives require the special syntax '<alias> in <expression>'."
          })
          return
        }
