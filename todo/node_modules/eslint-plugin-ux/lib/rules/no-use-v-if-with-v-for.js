...
/**
 * Check whether the given `if` node is using the variable which is defined by the `for` directive.
 * @param {ASTNode} vIf The `if` attribute node to check.
 * @returns {boolean} `true` if the `if` is using the variable which is defined by the `for` directive.
 */
...
module.exports = {
  meta: {
    type: 'suggestion',
    docs: {
      description: 'disallow use if on the same element as for',
      category: 'essential',
          ...
          if (isUsingIterationVar(node)) {
            if (!allowUsingIterationVar) {
              ...
              context.report({
                ...
                loc: node.loc,
                message:
                  "The '{{iteratorName}}' variable inside 'for' directive should be replaced with a computed property that returns filtered array instead. You should not mix 'for' with 'if'.",
                data: {
            ...
            context.report({
              node,
              loc: node.loc,
              message: "This 'if' should be moved to the wrapper element."
            })
          }
        }
      }
