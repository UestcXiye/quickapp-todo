    ...
    return utils.defineTemplateBodyVisitor(context, {
      "VAttribute[directive=true][key.name.name='on']" (node) {
        ...
        if (
          ...
          if (node.value && node.key.argument.name === 'click' && !node.value.expression) {
            context.report({
              node,
              loc: node.loc,
              message: "'@click' directives require an event name."
            })
          } else if (node.value && sourceCode.getText(node.value.expression)) {
